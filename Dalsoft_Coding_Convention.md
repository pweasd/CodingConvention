# Dalsoft Coding Convention Guide

달소프트 코딩 컨벤션 가이드는 [Vue.js](https://kr.vuejs.org/v2/guide/index.html) 개발을 기준으로 하며

[Airbnb 자바스크립트 스타일 가이드](https://github.com/airbnb/javascript)와 [Airbnb CSS / Sass 스타일 가이드](https://github.com/CodeMakeBros/css-style-guide)를 참고하여 작성되었습니다.
...
마크업 가이드의 내용은 Sass 그중에서 SCSS문법을 기준으로 하며 OOCSS와 BEM의 혼용을 권장합니다.

> [SCSS를 기준으로 하는 이유](https://designmeme.github.io/ko/blog/write-sass-with-scss/)
...
다음과 같은 이유로 OOCSS와 BEM의 혼용을 권장합니다:
...
<u>PascalCase 형태의 블록과 함께 사용하는 BEM의 변형 방식을 권장합니다. 이는 React, Vue에서 쓰이는 컴포넌트 등과 결합할 때 효과적입니다.</u> 이 경우에도 `_`와 `-`는 계속 변형자와 자식을 나타내기 위해 사용됩니다.
...
```html
<!-- ListingCard.vue (component) -->
<template>
  	<div class="ListingCard ListingCard--big">
...
- `.ListingCard`는 "블록"이며 더 높은 단계의 컴포넌트를 나타냅니다. ex) area 나 container 영역
- `.ListingCard__title`는 "요소"이며 그 블록을 전체로 조정하도록 돕는 `.ListingCard`의 자손임을 나타냅니다
- `.ListingCard--big`은 "변형자"이며 '.ListingCard' 블록에 존재하는 다른 상태 또는 변형을 나타냅니다



> ### ※ 중요
>
> OOCSS는 양날의 검과 같습니다. 매우 유용하지만 잘못 사용할 경우 문제가 생길 수 있습니다.
>
> 이전에는 A,B,C의 블록이 유사하여 공통 class를 통해 스타일을 적용시켰지만, 자주 변화하는 요구사항들에 따라 해당 블록들은 언제든지 나눠질 수 있습니다. 이때 해당 블록에 포함된 자손이 많거나 너무 과하게 혹은 잘못 혼용되었다면 심각한 사이드 이펙트와 스타일의 수정을 야기합니다.
>
> 그러므로 공통 class의 스타일은 최소한으로 정의되어야 하며 남용되지 말아야합니다.
...
- 클래스 이름에는 camelCase 방식보다 BEM 방식을 사용하세요.
- ID 선택자를 사용하지 마세요.
...
* `scoped`를 반드시 사용해주세요. 자손 컴포넌트의 css에 접근할 때는 `/deep/`연결자를 사용해주세요.
...
Mixin은 코드를 DRY("Don't Repeat Yourself")하게 하고 명료하게 하며, 복잡성을 줄이기 위해 사용해야 합니다. 인자를 받지 않는 Mixin은 이럴 때 유용합니다. 또한 OOCSS의 원칙을 보다 잘 구현하며 남용을 예방하고 font, button과 같은 스타일 형태를 정의하여 편리하게 사용 할 수 있습니다.

>  `@extend`는 직관적이지 않고 특히 중첩 선택자와 함께 사용할 때 위험성이 있기 때문에 사용하지 않는 것을 권장합니다. 심지어 최상위 placeholder 선택자를 extend해도 선택자들의 순서가 바뀌게 되면 문제가 발생 할 수 있습니다. 따라서 mixin을 사용하면 됩니다.
>
>  [Mixins을 권장하는 이유](https://mytory.net/2016/12/23/when-to-use-extend-when-to-use-a-mixin.html)
...
자바스크립트 가이드의 내용은 ES6의 문법을 기준으로 하며 Babel polyfill, [lodash](https://lodash.com/) 적용을 가정합니다.
...
**[⬆ back to top](#Dalsoft Coding Convention Guide)**
